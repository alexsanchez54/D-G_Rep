...
for (i in 1:nrow(myData)) {
     if ("control_learn2" %in% sessionTasks[sessionTasks$session_id == myData$session_id[i], "task_id"]) {
          myData$learnCond[i] <- 1
     }
     else if ("exp_learn2" %in% sessionTasks[sessionTasks$session_id == myData$session_id[i], "task_id"]) {
